...
describe('Root.InsuranceAPI - Policy', () => {
    ...
    it('#addPolicyBeneficiary()', (done) => {
        moxios.withMock(() => {
            const onFulfilled = sinon.spy();
            const stubData = {};
            client.addPolicyBeneficiary('bf1ada91-eecb-4f47-9bfa-1258bb1e0055', { type: 'id', number: '8704094800082', country: 'ZA' }, 'Jared', 'Dunn', 100).then((result) => {
                assert.deepEqual(stubData, result);
            }).then(onFulfilled);
            moxios.wait(helper.mock.bind(this, onFulfilled, stubData, done));
        });
    ...
    it('#replacePolicy()', (done) => {
        moxios.withMock(() => {
            const onFulfilled = sinon.spy();
            const stubData = {};
            client.replacePolicy('128ba0c0-3f6a-4f8b-9b40-e2066b02b59e', '8b922b9d-e874-4b06-87d0-e1f16a2d57db').then((result) => {
                assert.deepEqual(stubData, result);
            }).then(onFulfilled);
            moxios.wait(helper.mock.bind(this, onFulfilled, stubData, done));
        });
    ...
    it('#updatePolicyBillingAmount()', (done) => {
        moxios.withMock(() => {
            const onFulfilled = sinon.spy();
            const stubData = {};
            client.updatePolicyBillingAmount('128ba0c0-3f6a-4f8b-9b40-e2066b02b59e', 450 * 10).then((result) => {
                assert.deepEqual(stubData, result);
            }).then(onFulfilled);
            moxios.wait(helper.mock.bind(this, onFulfilled, stubData, done));
        });
